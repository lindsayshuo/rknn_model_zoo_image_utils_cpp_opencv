...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color) {
    ...
    if (drect.width != dstWidth || drect.height != dstHeight) {
        ...
        color_union.color_bytes[0] = color;
        color_union.color_bytes[1] = color;
        color_union.color_bytes[2] = color;
        color_union.color_bytes[3] = color;

        printf("fill dst image (x y w h)=(%d %d %d %d) with color=0x%x\n",
            dst_whole_rect.x, dst_whole_rect.y, dst_whole_rect.width, dst_whole_rect.height, color_union.imcolor);

        ret_rga = imfill(rga_buf_dst, dst_whole_rect, color_union.imcolor);
        if (ret_rga <= 0) {
            if (dst != NULL) {
...
image_format_t get_image_format(int channels) {
    switch (channels) {
        case 1:
            return IMAGE_FORMAT_GRAY8;
        case 3:
            return IMAGE_FORMAT_RGB888; // Assuming BGR to RGB conversion if needed
        case 4:
            return IMAGE_FORMAT_RGBA8888;
        default:
            return IMAGE_FORMAT_GRAY8; // Default to grayscale
    }
}
...
int read_image(const std::string& image_path, image_buffer_t* image_buffer) {
    ...
    if (!src_image.data) {
        std::cerr << "cv::imread " << image_path << " fail!" << std::endl;
        return -1;
    }

    image_buffer->width = src_image.cols;
    image_buffer->height = src_image.rows;
    image_buffer->width_stride = src_image.step[1];
    image_buffer->height_stride = src_image.step[0];
    image_buffer->format = get_image_format(src_image.channels());
    image_buffer->size = src_image.total() * src_image.elemSize();
    image_buffer->virt_addr = new unsigned char[image_buffer->size];
    
    // Copy data from cv::Mat to image_buffer_t
    std::memcpy(image_buffer->virt_addr, src_image.data, image_buffer->size);

    image_buffer->fd = -1; // If you have a file descriptor, set it appropriately

    return 0;
}
int write_image(const std::string& image_path, const image_buffer_t* image_buffer) {
    if (!image_buffer->virt_addr) {
    ...
    cv::Mat img(image_buffer->height, image_buffer->width, img_type, image_buffer->virt_addr);

    // Write the image using OpenCV
    bool success = cv::imwrite(image_path, img);
    if (!success) {
        std::cerr << "Failed to write image to " << image_path << std::endl;
        return -1;
    }
